...
function saveDrawing(document: DrawingDocument) {
	saveComponent({ title: title.value.trim() || "Drawing", drawing: document });
}
function linkSourceChanged() {
	...
	try {
		...
		if (metadataAbort === controller && editing.value)
			error.value = controller.signal.aborted
				? "The metadata request timed out. You can still save this link."
				: `${message(cause)} You can still save this link.`;
	} finally {
		window.clearTimeout(timeout);
		if (metadataAbort === controller) busy.value = false;
	}
}
function saveLink() {
	const url = webURL(source.value.trim());
		...
		>
			...
			<template v-else>
				...
				<span class="draft-controls"
					><button
						type="button"
						:disabled="busy || !webURL(source.trim())"
						@click="fetchMetadata"
					>
						{{ busy ? "Fetching…" : "Fetch preview" }}</button
					><span class="hint"
						>Contacts this site to discover metadata and video.</span
					></span
				>
