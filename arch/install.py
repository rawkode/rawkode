...
boot_partition = disk.PartitionModification(
    ...
    length=disk.Size(
        boot_partition_size, disk.Unit.GiB, device.device_info.sector_size
    ),
...
root_start = disk.Size(
    boot_partition_size + 1, disk.Unit.GiB, device.device_info.sector_size
)
...
locale_config = locale.LocaleConfiguration(
    kb_layout="us", sys_enc="UTF-8", sys_lang="en_GB"
)
...
network_config = models.NetworkConfiguration(type=models.NicType.NM)

audio_config = models.AudioConfiguration(audio=models.Audio.Pipewire)
...
with Installer(
    mountpoint, disk_config, disk_encryption=disk_encryption, kernels=kernels
) as installation:
