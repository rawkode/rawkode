...
// recurrence metadata, and sync timestamps. A validated person node id is retained only as a
// safe navigation handle for the local second-brain graph.
...
public struct TodayBriefPerson: Codable, Hashable, Sendable {
    ...
    private enum CodingKeys: String, CodingKey { case displayName, personNodeId }

    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        guard container.contains(.displayName) else {
            displayName = nil
            personNodeId = try container.decodeIfPresent(EntityId.self, forKey: .personNodeId)
            return
        }
        do {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        if let personNodeId {
            try container.encode(personNodeId, forKey: .personNodeId)
        }
    }
}
...
public struct TodayBriefEvent: Codable, Hashable, Sendable {
    ...
    public init(id: EntityId, occurrenceKey: String, title: String, start: IsoDateTimeString, end: IsoDateTimeString, people: [TodayBriefPerson]) {
        self.id = id
...
/// A server-owned preparation request. Native callers name only the safe occurrence handle and
/// deterministic daily-note identity; the Worker resolves the event and produces the Loro edit.
public struct PrepareMeetingInDailyNoteInput: Hashable, Sendable {
    ...
    public init(workspaceId: EntityId, dailyNoteId: EntityId, localDate: LocalDate, timeZone: IanaTimeZone, occurrenceKey: String, intent: LoroMutationIntentV1) throws {
        guard occurrenceKey.count == 64,
              occurrenceKey.unicodeScalars.allSatisfy({ (48...57).contains($0.value) || (97...102).contains($0.value) }) else {
            throw TodayBriefPreparationError.invalidInput
        }
...
public struct PrepareMeetingInDailyNoteOutput: Hashable, Sendable {
    public enum Status: String, Hashable, Sendable { case created, alreadyPrepared }
    public let dailyNoteId: EntityId
    public let localDate: LocalDate
    public let occurrenceKey: String
    ...
    public init(dailyNoteId: EntityId, localDate: LocalDate, occurrenceKey: String, status: Status, resultSnapshotSha256: String) throws {
        guard occurrenceKey.count == 64,
              occurrenceKey.unicodeScalars.allSatisfy({ (48...57).contains($0.value) || (97...102).contains($0.value) }),
              LoroMutationWire.isDigest(resultSnapshotSha256) else {
            throw TodayBriefPreparationError.invalidOutput
        }
        self.dailyNoteId = dailyNoteId
        self.localDate = localDate
        self.occurrenceKey = occurrenceKey
        self.status = status
        self.resultSnapshotSha256 = resultSnapshotSha256
    }
}

public enum TodayBriefPreparationError: Error, Sendable, Equatable {
    case invalidInput
    case invalidOutput
}
