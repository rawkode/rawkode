...
describe("Today Brief route ownership", () => {
  ...
  it("makes the daily note the route's primary document before its context aside", () => {
    ...
    expect(dailyNoteSource).toContain("<DailyStandup dailyNoteId={state.value.nodeId} includeLedger={isToday} />")
    expect(ledgerActivitySource).toContain("export function DailyStandup({ dailyNoteId, includeLedger = true }")
