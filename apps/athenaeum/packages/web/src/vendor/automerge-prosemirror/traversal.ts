...
class TraverseState {
  ...
  *newBlock(block: BlockMarker): IterableIterator<TraversalEvent> {
    if (block.isEmbed) {
      ...
      if (!content.isInline) {
        // Deliberate vendoring edit #3 (see `VENDOR.md`): upstream's isEmbed handling assumes the
        // embed is an INLINE leaf — its own only isEmbed user, `image`, is inline — meant to sit
        // inside whichever textblock is already open on `this.stack`, so it never closes that
        // frame first. A BLOCK-level isEmbed leaf (this schema's `horizontal_rule`, added in
        // `../../rich-text/schema.ts` — upstream's own `basicSchema.ts` never maps `divider` at
        // all) breaks that assumption: if a textblock is left open on the stack (e.g. an empty
        // paragraph immediately before the divider — the common case right after pressing Enter
        // out of a block), `this.currentMatch` below resolves to that paragraph's own "inline*"
        // content model, and matching a block-group node against it returns null, which the
        // `currentMatch` setter turns into a thrown "Match cannot be null" — 100% reproducible on
        // every reload of a note containing a divider (confirmed live: `traverseSpans`, which this
        // method drives, only runs when rebuilding a PM doc FROM STORED SPANS on mount/reload; a
        // live local `replaceRangeWith` bypasses this code entirely, so the bug was invisible
        // until a real reload-and-resync was exercised). Fix: for a non-inline embed, run the same
        // close-mismatched-frames-then-reopen dance the non-embed branch below already does,
        // before matching the embed's own type, so it's matched against the correct (block-level)
        // content model instead of whatever textblock happened to be left open.
