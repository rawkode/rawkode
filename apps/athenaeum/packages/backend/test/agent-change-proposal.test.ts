...
describe("agent change proposal capture and reservations", () => {
  ...
  afterEach(() => {
    ...
    workspace = undefined
    decisionWorkspace?.stub[Symbol.dispose]()
  ...
  it("accepts a captured proposal through the authenticated ledger gate exactly once", async () => {
    const { workspaceId, chat } = await pendingChat()
    const native = workspaceDurableObjectStub(workspaceId)
    const proposal = Schema.decodeUnknownSync(AgentChangeProposal)(
      await native.debugCaptureAgentChangeProposal(captureInput(chat.id, "decision-capture-1"))
    )
    const { credential } = await devSignIn(`decision-${crypto.randomUUID()}@example.com`)
    decisionWorkspace = await connectToWorkspaceWithSocketAs(workspaceId, credential)

    const input = new DecideAgentChangeProposalInput({
      workspaceId,
      proposalId: proposal.proposalId,
      requestId: "decision-request-1",
      decision: "accept",
      message: "Promote the captured enrichment after checking the evidence.",
      provenance: "employee:enrichment/job:attendee-profile/run:1"
    })
    const encoded = Schema.encodeSync(DecideAgentChangeProposalInput)(input)
    const first = Schema.decodeUnknownSync(DecideAgentChangeProposalOutput)(
      await decisionWorkspace.stub.decideAgentChangeProposal(encoded)
    )
    const replay = Schema.decodeUnknownSync(DecideAgentChangeProposalOutput)(
      await decisionWorkspace.stub.decideAgentChangeProposal(encoded)
    )
    expect(first).toEqual(replay)
    expect(first).toMatchObject({ proposalId: proposal.proposalId, state: "accepted" })
    await expect(decisionWorkspace.stub.decideAgentChangeProposal(
      Schema.encodeSync(DecideAgentChangeProposalInput)(new DecideAgentChangeProposalInput({
        ...input,
        message: "A different reason must not reuse the same request identity."
      }))
    )).rejects.toThrow()
    ...
    const requestIdentity = "agent-change-decision:decision-request-1"
    expect(await native.debugGetLedgerCommand(requestIdentity)).toMatchObject({
      type: "agentChangeDecision",
      principal: expect.stringContaining("decision-"),
      message: input.message,
      payload: { proposalId: proposal.proposalId, decision: "accept" }
    })
    expect(await native.debugGetLedgerReceipt(requestIdentity)).toMatchObject({ output: { state: "accepted" } })
    expect(await native.debugGetLedgerEvent(requestIdentity)).toMatchObject({
      kind: "agent-change-decision",
      payload: { proposalId: proposal.proposalId, decision: "accept", output: { state: "accepted" } }
    })
    expect(await native.debugGetLedgerOutboxIntent(requestIdentity)).toMatchObject({
      kind: "agent-change-decision",
      payload: { proposalId: proposal.proposalId, decision: "accept", output: { state: "accepted" } }
    })
  })

  it("marks a proposal conflicted without partial writes when its snapshot goes stale", async () => {
    const { workspaceId, chat } = await pendingChat()
    const native = workspaceDurableObjectStub(workspaceId)
    const proposal = Schema.decodeUnknownSync(AgentChangeProposal)(
      await native.debugCaptureAgentChangeProposal(captureInput(chat.id, "decision-capture-stale"))
    )
    await workspace!.mergeChanges(Schema.encodeSync(MergeChangesInput)(new MergeChangesInput({ chatId: chat.id, mergeThrough: 0 })))

    const { credential } = await devSignIn(`stale-${crypto.randomUUID()}@example.com`)
    decisionWorkspace = await connectToWorkspaceWithSocketAs(workspaceId, credential)
    const result = Schema.decodeUnknownSync(DecideAgentChangeProposalOutput)(await decisionWorkspace.stub.decideAgentChangeProposal(
      Schema.encodeSync(DecideAgentChangeProposalInput)(new DecideAgentChangeProposalInput({
        workspaceId,
        proposalId: proposal.proposalId,
        requestId: "decision-request-stale",
        decision: "reject",
        message: "Reject this proposal if and only if its captured state is still current.",
        provenance: "employee:review/job:stale-check/run:1"
      }))
    ))
    expect(result).toMatchObject({ proposalId: proposal.proposalId, state: "conflicted" })
    const preserved = Schema.decodeUnknownSync(GetNodeOutput)(await workspace!.getNode(
      Schema.encodeSync(GetNodeInput)(new GetNodeInput({ workspaceId, nodeId: proposal.snapshot[0]!.id as EntityId }))
    ))
    expect(preserved.node.pending).toBeUndefined()
  })
