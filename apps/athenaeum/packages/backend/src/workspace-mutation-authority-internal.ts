...
import {
  AUTHORITY_VERSION, PUBLIC_REQUEST_VERSION, RECEIPT_VERSION, actorCustodyMaterial, authorityFingerprint, maybeThenable, nonBlank,
...
export const executeMutationAuthorityWithRegistry = async <Output = unknown>(
  ...
  for (let attempt = 0; attempt < Math.max(1, Math.trunc(handlerAttempts)); attempt++) {
    ...
    const result = store.transactionSync((transaction): AuthorityOutcome<Output> | Readonly<{ kind: "retry" }> => {
      ...
      const event = Object.freeze({ eventId, workspaceId: prepared.workspaceId, epoch, sequence, commandFingerprint, digest: syncDigest({ version: AUTHORITY_VERSION, eventId, workspaceId: prepared.workspaceId, epoch, sequence, commandFingerprint, causationId: prepared.causationId, correlationId: prepared.correlationId, createdAt, payload: eventPayload }), causationId: prepared.causationId, correlationId: prepared.correlationId, createdAt, payload: eventPayload })
      const receipt: AuthorityReceipt<Output> = Object.freeze({ version: RECEIPT_VERSION, workspaceId: prepared.workspaceId, requestId: prepared.requestId, commandFingerprint, epoch, sequence, output: freezeLocalMutationInput(output), stagedIntentCount: staged.length })
      ...
      for (const intent of staged) { const outboxId = identity.nextOutboxId(); const outbox = Object.freeze({ outboxId, eventId, workspaceId: prepared.workspaceId, epoch, sequence, commandFingerprint, digest: syncDigest({ version: AUTHORITY_VERSION, outboxId, eventId, workspaceId: prepared.workspaceId, epoch, sequence, commandFingerprint, recipient: intent.recipient, payload: intent.payload, createdAt }), recipient: intent.recipient, payload: intent.payload, createdAt }); transaction.insertOutboxIntent(outbox); transaction.hitFailpoint("after-outbox"); transaction.insertDeliverySeed({ outboxId, consumerId: intent.recipient, state: "pending", attempts: 0 }); transaction.hitFailpoint("after-delivery") }
