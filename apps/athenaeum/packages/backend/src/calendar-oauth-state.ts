...
import { CalendarOAuthStateNonceDigest } from "./calendar-connection-identity.js"

const textEncoder = new TextEncoder()
const STATE_VERSION = "athenaeum-calendar-oauth-state-v1"
const ATTEMPT_STATE_VERSION = "athenaeum-calendar-oauth-state-v2"
...
/**
 * V2 deliberately keeps the redirect state privacy-safe: it has only a random nonce and time
 * bounds. Workspace/principal/connection/binding identities live exclusively in the private DO
 * row addressed by the nonce digest.
 */
interface AttemptStatePayload {
  readonly v: typeof ATTEMPT_STATE_VERSION
...
export const makeCalendarOAuthStateNonce = (): string => {
  const bytes = crypto.getRandomValues(new Uint8Array(32))
  return base64UrlEncode(bytes)
}

/** The raw nonce is only ever in the signed redirect state; storage indexes its SHA-256 digest. */
export const digestCalendarOAuthStateNonce = (nonce: string): Effect.Effect<CalendarOAuthStateNonceDigest, Unauthorized> =>
  Effect.gen(function* () {
    if (!/^[A-Za-z0-9_-]{32,128}$/.test(nonce)) {
      return yield* Effect.fail(new Unauthorized({ message: "Malformed OAuth state nonce." }))
    }
    const digest = yield* Effect.promise(() => crypto.subtle.digest("SHA-256", textEncoder.encode(nonce)))
    const hex = [...new Uint8Array(digest)].map((byte) => byte.toString(16).padStart(2, "0")).join("")
    return Schema.decodeUnknownSync(CalendarOAuthStateNonceDigest)(hex)
  })

export const mintCalendarOAuthAttemptState = (
  nonce: string,
  secret: string,
  now: Date = new Date()
): Effect.Effect<string, Unauthorized> =>
  Effect.gen(function* () {
    // Validate locally before signing; this also ensures callers cannot put incidental identifiers in the nonce.
    yield* digestCalendarOAuthStateNonce(nonce)
    const iat = Math.floor(now.getTime() / 1000)
    const payload: AttemptStatePayload = { v: ATTEMPT_STATE_VERSION, nonce, iat, exp: iat + STATE_TTL_SECONDS }
    const payloadBytes = textEncoder.encode(JSON.stringify(payload))
    const key = yield* Effect.promise(() => importHmacKey(secret, "sign"))
    const signature = new Uint8Array(yield* Effect.promise(() => crypto.subtle.sign("HMAC", key, payloadBytes)))
    return `${base64UrlEncode(payloadBytes)}.${base64UrlEncode(signature)}`
  })

export const verifyCalendarOAuthAttemptState = (
  state: string,
  secret: string,
  now: Date = new Date()
): Effect.Effect<{ readonly nonce: string; readonly nonceDigest: CalendarOAuthStateNonceDigest }, Unauthorized> =>
  Effect.gen(function* () {
    ...
    if (
      ...
      parsed === null ||
      (parsed as Record<string, unknown>).v !== ATTEMPT_STATE_VERSION ||
    ...
    const payload = parsed as AttemptStatePayload
    if (Math.floor(now.getTime() / 1000) >= payload.exp) {
      return yield* Effect.fail(new Unauthorized({ message: "OAuth state has expired." }))
    }
    const nonceDigest = yield* digestCalendarOAuthStateNonce(payload.nonce)
    return { nonce: payload.nonce, nonceDigest }
  })
