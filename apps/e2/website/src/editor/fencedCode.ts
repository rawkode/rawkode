...
export const fenceAuthoringKey = new PluginKey<ReadonlyMap<number, number>>(
	"fieldnotesFences",
);
type FenceAction = { open: { position: number; length: number } } | {
	close: number;
};
...
export const parseCompletedFences = (text: string): CompletedFence[] => {
...
const fencedNode = (
	...
	if (token === "d2" || token === "mermaid") {
		const component = {
			...defaultComponent(token === "d2" ? "diagram" : "mermaid"),
			source,
		};
		return schema.nodes.paragraph!.create(
			null,
			schema.nodes.component!.create({ component }),
		);
	}
...
const protectedSelection = (state: EditorState): boolean => {
...
export const createFencedPasteTransaction = (
	state: EditorState,
	text: string,
	...
	return closeHistory(state.tr).replaceSelection(slice).setMeta("paste", true)
		.setMeta("uiEvent", "paste").scrollIntoView();
};

/** Enter closes only a code block opened during this editing session. */
export const createFenceEnterTransaction = (
	state: EditorState,
): Transaction | undefined => {
	const { $from, empty } = state.selection;
	if (
		!empty || !$from.depth || $from.parentOffset !== $from.parent.content.size
	) return undefined;
	...
	const transaction = closeHistory(state.tr).replaceWith(
		from,
		from + parent.nodeSize,
		type.create({ language: opening[2]!.trim() }),
	)
		.setMeta(
			fenceAuthoringKey,
			{
				open: { position: from, length: opening[1]!.length },
			} satisfies FenceAction,
		);
...
export const createFencedCodePlugin = (): Plugin<
	...
	return new Plugin<ReadonlyMap<number, number>>({
		...
		state: {
			...
			apply(transaction, previous, _oldState, newState) {
				const action = transaction.getMeta(fenceAuthoringKey) as
					| FenceAction
					| undefined;
				if (!transaction.docChanged && !action) return previous;
				for (const [position, length] of previous) {
					if (action && "close" in action && action.close === position) {
						continue;
					}
				...
				if (action && "open" in action) {
					...
					if (node?.type.name === "codeBlock") {
						known.set(node, action.open.length);
					}
				}
		...
		props: {
			handlePaste(view, event) {
				...
				try {
					...
					return false; /* Default paste preserves literal source if the schema cannot fit it. */
				}
			},
		},
	});
			...
			new InputRule({
				...
				handler: ({ state, range, match }) => {
					...
					if (
						...
						start.parentOffset !== 0 || range.to !== start.end() ||
					...
					state.tr.delete(range.from, range.to).setBlockType(
						range.from,
						range.from,
						type,
						{ language: match[2]!.trim() },
						...
						.setMeta(
							fenceAuthoringKey,
							{
								open: { position: start.before(), length: match[1]!.length },
							} satisfies FenceAction,
						);
				},
			}),
