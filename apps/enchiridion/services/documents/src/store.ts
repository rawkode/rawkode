...
export class DocumentStore {
	...
	async load(key: string): Promise<DocumentSnapshot | null> {
		...
		const row = await this.db
			.prepare(
				'SELECT revision, snapshot, format, extensions, updated_at FROM documents WHERE owner = ? AND key = ?',
			)
			.bind(this.owner, key)
			.first<{
				revision: number;
	...
	async save(input: SaveDocument) {
		...
		const result = await this.db
			.prepare(
				`INSERT INTO documents(owner, key, revision, mutation_id, snapshot, format, extensions, updated_at)
      ...
      WHERE documents.revision = ?`,
		...
		if (result.meta.changes === 1)
			return { revision: input.expectedRevision + 1, updatedAt: now };
		const prior = await this.db
			.prepare(
				'SELECT revision, updated_at, mutation_id, snapshot FROM documents WHERE owner = ? AND key = ?',
			)
			.bind(this.owner, input.key)
			.first<{
				revision: number;
				updated_at: number;
				mutation_id: string;
		...
		if (
			...
			prior.snapshot === input.snapshot
		)
			return { revision: prior.revision, updatedAt: prior.updated_at };
		throw new Error(
			'DOCUMENT_CONFLICT: Another editor saved this document. Your local copy is retained; export it before reloading.',
		);
	}
}
