...
Install Deno 2.9+ and Node 22.15+ (Node 24+ recommended). Run from
`apps/enchiridion`:
...
Run from `apps/enchiridion` with 1Password authentication available to cuenv.
The configured Cloudflare token must cover the account that owns both zones:
