...
import type {
	Connection,
	OAuthIntegrationApi,
} from "@enchiridion/oauth-client";
import type { AccountEnv } from "./env.ts";
import { calendarEndpoint } from "./env.ts";
import { authorized, scopes } from "./google.ts";
...
export const syncCalendar = async (
	...
	const db = env.DB;
	await db.prepare(
		"INSERT OR IGNORE INTO calendar_syncs (connection_id, owner_id) VALUES (?, ?)",
	).bind(id, connection.ownerId).run();
	const run = crypto.randomUUID();
	const cursor = await db.prepare(
		`UPDATE calendar_syncs SET lease = ?, lease_until = ? WHERE connection_id = ?
    AND (lease_until IS NULL OR lease_until < ?) RETURNING sync_token, synced_at`,
	).bind(run, Date.now() + 120_000, id, Date.now()).first<Cursor>();
	if (!cursor) {
		throw new CalendarError("This calendar is already syncing. Retry shortly.");
	}
	...
	try {
		...
		for (let page = 0; page < 200; page++) {
			const token = await oauth.getAccessToken(id, [
			...
			if (response.status === 410 && syncToken && !restarted) {
				...
				restarted = true;
				changed = 0;
				await db.prepare("DELETE FROM calendar_staging WHERE run_id = ?").bind(
					run,
				).run();
				continue;
			}
			if (!response.ok) {
				throw new CalendarError(
					response.status === 401
						? "Google rejected access. Reconnect the account."
						: "Google Calendar could not be synced. Retry shortly.",
				);
			}
			...
			if (body.items.length) {
				await db.batch(
					body.items.map((event) =>
						db.prepare(
							"INSERT OR REPLACE INTO calendar_staging (run_id, event_id, data, cancelled) VALUES (?, ?, ?, ?)",
						)
							.bind(
								run,
								event.id,
			...
			const renewed = await db.prepare(
				"UPDATE calendar_syncs SET lease_until = ? WHERE connection_id = ? AND lease = ? AND lease_until > ?",
			)
				.bind(Date.now() + 120_000, id, run, Date.now()).run();
			if (!renewed.meta.changes) {
				throw new CalendarError("The sync lease expired. Retry the sync.");
			}
			if (body.nextPageToken) {
				if (
					typeof body.nextPageToken !== "string" ||
					body.nextPageToken === pageToken
				) throw new CalendarError("Google returned an invalid page cursor.");
		...
		if (!nextSyncToken || typeof nextSyncToken !== "string") {
			throw new CalendarError(
				"The calendar sync did not finish. Retry shortly.",
			);
		}
		...
		const now = Date.now();
		const gate =
			"EXISTS (SELECT 1 FROM calendar_syncs WHERE connection_id = ? AND lease = ? AND lease_until > ?)";
		const result = await db.batch([
			db.prepare(
				`DELETE FROM calendar_events WHERE connection_id = ? AND ${gate}
        ...
        ON CONFLICT(connection_id, event_id) DO UPDATE SET data = excluded.data`)
				.bind(id, run, id, run, now),
			db.prepare(
				"UPDATE calendar_syncs SET sync_token = ?, synced_at = ?, lease = NULL, lease_until = NULL WHERE connection_id = ? AND lease = ? AND lease_until > ?",
			)
		...
		await db.batch([
			...
			db.prepare(
				"UPDATE calendar_syncs SET lease = NULL, lease_until = NULL WHERE connection_id = ? AND lease = ?",
			).bind(id, run),
		]);
	}
};
