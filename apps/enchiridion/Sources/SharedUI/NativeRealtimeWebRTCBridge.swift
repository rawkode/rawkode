  ...
  private struct NativeBridgeToken: Sendable, Equatable { let generation: UInt64; let epoch: UInt64 }
  ...
  final class NativeRealtimeWebRTCBridge: NSObject, RealtimeWebRTCBridging {
    ...
    func load() throws {
      ...
      if !emittedReady { emittedReady = true; relay.push(.ready) }
    }

    func authorize(_ capability: RealtimeWebRTCBridgeAuthorization, generation: UInt64) async throws {
      guard let epoch = lifecycle.beginAuthorization(), capability.authorizes(generation: generation) else { throw RealtimeVoiceTransportError.bridgeClosed }
      tearDown(epoch: epoch); authorization.revoke()
    ...
    func start(generation: UInt64) async throws {
      guard let epoch = lifecycle.currentEpoch, authorization.authorizes(generation: generation) else { throw RealtimeVoiceTransportError.bridgeClosed }
      let token = NativeBridgeToken(generation: generation, epoch: epoch)
      tearDown(epoch: epoch)
      ...
      self.peer = peer; self.transceiver = transceiver; self.channel = channel; self.peerProxy = proxy; self.channelProxy = channelProxy
      ...
      do {
        ...
        relay.push(.offer(generation: generation, sdp: final))
    ...
    func applyAnswer(_ sdp: String, generation: UInt64) async throws {
      guard let epoch = lifecycle.currentEpoch else { throw RealtimeVoiceTransportError.bridgeClosed }
      let token = NativeBridgeToken(generation: generation, epoch: epoch)
      guard isCurrent(token), sdp.utf8.count <= Self.maximumSDPBytes, let peer else { throw RealtimeVoiceTransportError.bridgeClosed }
      ...
      relay.push(.answerApplied(generation: generation))
    }

    func sendEvent(_ json: String, generation: UInt64) async throws {
      guard let epoch = lifecycle.currentEpoch, isCurrent(.init(generation: generation, epoch: epoch)), json.utf8.count <= RealtimeProtocolCodec.maximumEventBytes, let channel, channel.readyState == .open, channel.sendData(.init(data: Data(json.utf8), isBinary: false)) else { throw RealtimeVoiceTransportError.bridgeClosed }
    }

    func setInputEnabled(_ enabled: Bool, lease: RealtimeVoiceInputLease) async throws {
      guard let epoch = lifecycle.currentEpoch, isCurrent(.init(generation: lease.transportGeneration, epoch: epoch)), newestLease.map({ lease.inputEpoch > $0.inputEpoch }) ?? true, let sender = transceiver?.sender else { throw RealtimeVoiceTransportError.bridgeClosed }
      newestLease = lease
      ...
      guard newestLease == lease, isCurrent(.init(generation: lease.transportGeneration, epoch: epoch)) else { throw RealtimeVoiceTransportError.bridgeClosed }
      sender.track = track; inputSource = source; inputTrack = track
    }

    func stop() async { guard let epoch = lifecycle.stop() else { return }; authorization.revoke(); tearDown(epoch: epoch); relay.finish() }
    ...
    private func isCurrent(_ token: NativeBridgeToken) -> Bool { lifecycle.isCurrent(token.epoch) && authorization.authorizes(generation: token.generation) }
    private func terminal(_ token: NativeBridgeToken, _ reason: NativeBridgeReason) { guard isCurrent(token) else { return }; _ = lifecycle.beginAuthorization(); authorization.revoke(); tearDown(epoch: token.epoch); relay.terminal(.failure(generation: token.generation, code: reason.rawValue)) }
    private func tearDown(epoch: UInt64) { fence.invalidate(epoch); peerProxy?.detachOutput(); inputTrack = nil; inputSource = nil; transceiver?.sender.track = nil; channel?.delegate = nil; channel?.close(); channel = nil; channelProxy = nil; peer?.close(); peer = nil; peerProxy = nil; transceiver = nil; newestLease = nil }
  }
  ...
  final class NativeRealtimeWebRTCEventRelay: @unchecked Sendable {
    private let lock = NSLock(); private var controls: [RealtimeWebRTCBridgeEvent] = []; private var terminal: RealtimeWebRTCBridgeEvent?; private var activity: RealtimeWebRTCBridgeEvent?; private var waiter: CheckedContinuation<RealtimeWebRTCBridgeEvent?, Never>?; private var consumer = false; private var finished = false
    ...
    func finish() { lock.lock(); finished = true; let waiter = waiter; self.waiter = nil; lock.unlock(); waiter?.resume(returning: nil) }
  ...
  private final class NativePeerProxy: NSObject, LKRTCPeerConnectionDelegate, @unchecked Sendable {
    ...
    private let output = NativeOutputActivityPublisher(); private var iceGate: NativeRealtimeWebRTCOperationGate<Void>?; private var track: LKRTCAudioTrack?; private var renderer: NativeRemoteAudioRenderer?
    ...
    func installICE(_ gate: NativeRealtimeWebRTCOperationGate<Void>) { iceGate = gate }
    ...
    func peerConnection(_: LKRTCPeerConnection, didChange state: LKRTCIceGatheringState) { if state == .complete { iceGate?.succeed(()) } }
    func peerConnection(_: LKRTCPeerConnection, didStartReceivingOn transceiver: LKRTCRtpTransceiver) { guard track == nil, let track = transceiver.receiver.track as? LKRTCAudioTrack, fence.isCurrent(token) else { return }; let renderer = NativeRemoteAudioRenderer(slot: output.slot); self.track = track; self.renderer = renderer; track.add(renderer); output.start(token: token, fence: fence, relay: relay) }
  ...
  private final class NativeChannelProxy: NSObject, LKRTCDataChannelDelegate, @unchecked Sendable {
    let token: NativeBridgeToken; let fence: NativeCallbackFence; let relay: NativeRealtimeWebRTCEventRelay; let terminal: @MainActor @Sendable (NativeBridgeToken, NativeBridgeReason) -> Void
    init(token: NativeBridgeToken, fence: NativeCallbackFence, relay: NativeRealtimeWebRTCEventRelay, terminal: @escaping @MainActor @Sendable (NativeBridgeToken, NativeBridgeReason) -> Void) { self.token = token; self.fence = fence; self.relay = relay; self.terminal = terminal }
    func dataChannelDidChangeState(_ channel: LKRTCDataChannel) { switch channel.readyState { case .connecting: relay.push(.dataChannelState(generation: token.generation, state: "connecting")); case .open: relay.push(.dataChannelState(generation: token.generation, state: "open")); case .closing, .closed: scheduleTerminal(.channelTerminal); default: scheduleTerminal(.channelUnknown) } }
    func dataChannel(_: LKRTCDataChannel, didReceiveMessageWith buffer: LKRTCDataBuffer) { guard !buffer.isBinary, buffer.data.count <= RealtimeProtocolCodec.maximumEventBytes, let json = String(data: buffer.data, encoding: .utf8), fence.isCurrent(token) else { return }; relay.push(.serverEvent(generation: token.generation, json: json)) }
    private func scheduleTerminal(_ reason: NativeBridgeReason) { let token = token; let terminal = terminal; Task { @MainActor in terminal(token, reason) } }
  }
  ...
  private final class NativeOutputActivityPublisher: @unchecked Sendable { let slot = NativeAudioLevelSlot(); private var timer: DispatchSourceTimer?; func start(token: NativeBridgeToken, fence: NativeCallbackFence, relay: NativeRealtimeWebRTCEventRelay) { guard timer == nil else { return }; let timer = DispatchSource.makeTimerSource(queue: DispatchQueue(label: "dev.rawkode.enchiridion.native-webrtc-meter")); timer.schedule(deadline: .now() + .milliseconds(50), repeating: .milliseconds(50)); timer.setEventHandler { [slot, token, fence, relay] in guard fence.isCurrent(token), let value = slot.take() else { return }; relay.push(.audioActivity(generation: token.generation, inputLevel: 0, outputLevel: value)) }; self.timer = timer; timer.resume() }; func stop() { timer?.cancel(); timer = nil } }
