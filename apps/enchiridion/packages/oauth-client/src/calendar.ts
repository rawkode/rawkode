...
export interface CalendarApi {
  ...
  mailStatus(connectionId: string): Promise<{ expiration: number; history_id: string; notified_at: number | null } | null>;
  listConnections(): Promise<Connection[]>;
  sync(connectionId: string): Promise<{ changed: number; syncedAt: number }>;
  listEvents(connectionId: string): Promise<{ events: CalendarEvent[]; syncedAt: number | null }>;
}
