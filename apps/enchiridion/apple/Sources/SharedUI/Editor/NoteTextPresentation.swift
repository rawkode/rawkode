...
/// projection in EnchiridionCore never reads presentation back, so these are safe
/// to recompute on every edit.
enum NoteTextPresentation {
    static func styled(_ text: AttributedString, base: NoteBaseFont, theme: EnchiridionTheme) -> AttributedString {
        var result = text
    ...
    static func foregroundColor(for attributes: AttributeContainer, theme: EnchiridionTheme) -> Color? {
    ...
    static func backgroundColor(for attributes: AttributeContainer, theme: EnchiridionTheme) -> Color? {
