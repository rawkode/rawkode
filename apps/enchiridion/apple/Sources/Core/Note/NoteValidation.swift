...
/// Validate the shared JSON contract before Codable can discard unknown
/// fields. This mirrors the Zod schema in `packages/documents/src/note.ts`
/// by walking the decoder directly; it is not another persisted model.
public enum NoteValidation {
    public static let byteLimit = 16 * 1024 * 1024
    public static let nodeLimit = 20_000
    public static let depthLimit = 32
    public static let componentLimit = 1_000
    ...
    private static func invalid(_ detail: String) -> NoteDocument.FormatError { .unsupportedContent(detail) }
    ...
    private static func children(_ object: Object, allowed: Set<String>, minimum: Int = 0, depth: Int, budget: Budget, plain: Bool = false, firstParagraph: Bool = false) throws {
        ...
        guard (content.count ?? 0) <= nodeLimit else { throw invalid("too many children") }
        var count = 0
    ...
    private static func node(_ decoder: Decoder, allowed: Set<String>, depth: Int, budget: Budget, plain: Bool = false) throws {
        budget.nodes += 1
        guard depth <= depthLimit, budget.nodes <= nodeLimit else { throw invalid("document exceeds node or depth limit") }
        ...
        default:
            try children(object, allowed: blocks, minimum: 1, depth: depth + 1, budget: budget)
        }
    }
    ...
    private static func marks(_ decoder: Decoder, plain: Bool) throws {
        ...
        while !array.isAtEnd {
            ...
            case "textStyle":
                ...
                if let size = try attrs.string("fontSize", max: 16, nullable: true) { guard isFontSize(size) else { throw invalid("font size") } }
                for key in ["color", "backgroundColor"] {
                    if let value = try attrs.string(key, max: 100, nullable: true) { guard isColor(value) else { throw invalid("color") } }
                }
    ...
    public static func isFontSize(_ size: String) -> Bool {
        guard matches(size, "^[0-9]+(?:\\.[0-9]+)?px$"), let number = Double(size.dropLast(2)) else { return false }
        return (1...512).contains(number)
    }
    ...
    public static func isColor(_ value: String) -> Bool {
        ...
        if names.contains(value) || matches(value, "^#(?:[0-9A-Fa-f]{3}|[0-9A-Fa-f]{4}|[0-9A-Fa-f]{6}|[0-9A-Fa-f]{8})$") { return true }
        let channel = "(?:25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9])"
        if value.count <= 80, matches(value, "(?i)^rgb\\(\\s*\(channel)\\s*,\\s*\(channel)\\s*,\\s*\(channel)\\s*\\)$") { return true }
        if value.count <= 100, matches(value, "(?i)^rgba\\(\\s*\(channel)\\s*,\\s*\(channel)\\s*,\\s*\(channel)\\s*,\\s*(?:0(?:\\.[0-9]+)?|1(?:\\.0+)?)\\s*\\)$") { return true }
        return false
    }

    /// Absolute HTTP(S) URL without credentials, whitespace or control characters; optionally `mailto:`.
    public static func validateURL(_ value: String, mail: Bool = false) throws {
        guard value.utf16.count <= 8192, matches(value, "^[^\\x00-\\x20\\x7f]+$"),
              let url = URLComponents(string: value), url.user == nil, url.password == nil else { throw invalid("unsafe URL") }
        if mail, url.scheme?.lowercased() == "mailto" { return }
        guard matches(value, "(?i)^https?://"), ["http", "https"].contains(url.scheme?.lowercased() ?? ""), !(url.host ?? "").isEmpty else {
            throw invalid("expected HTTP(S) URL")
        }
    }

    public static func httpURL(_ value: String, mail: Bool = false) -> URL? {
        (try? validateURL(value, mail: mail)) == nil ? nil : URL(string: value)
    }
    ...
    private static func component(_ decoder: Decoder, budget: Budget) throws {
        ...
        guard budget.componentIDs.insert(id).inserted, budget.componentIDs.count <= componentLimit else { throw invalid("duplicate or too many components") }
    ...
    private static func entity(_ decoder: Decoder) throws {
        ...
        if probe.contains(Key("version")) {
            let object = try Object(decoder, keys: ["version", "entityId", "fallbackLabel", "displayText", "presentation"])
            try object.integer("version", range: 1...1, required: true)
            _ = try identifier(object, key: "entityId")
            for key in ["fallbackLabel", "displayText"] {
                guard let label = try object.string(key, max: 1_000, required: true), !label.isEmpty else { throw invalid(key) }
            }
            try object.choice("presentation", ["link", "mention"], required: true)
            return
        }
        ...
        guard !id.isEmpty, matches(id, "^[^\\x00-\\x1f\\x7f]+$") else { throw invalid("entity id") }
        guard let label = try object.string("label", max: 1_000, required: true), !label.isEmpty else { throw invalid("entity label") }
        try object.url("avatarURL")
        _ = try object.string("meta", max: 4_096)
    }

    private static func identifier(_ object: Object, key: String = "id") throws -> String {
        let id = try object.string(key, max: 36, required: true)!
        guard NoteIdentifier.isValid(id) else { throw invalid("identifier") }
