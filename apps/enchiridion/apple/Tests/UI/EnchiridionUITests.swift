...
final class EnchiridionUITests: XCTestCase {
    ...
    func testTasksCaptureRemainsVisibleOffline() throws {
        ...
        XCTAssertEqual(app.staticTexts.matching(identifier: "Keep this task offline").count, 1)
        XCTAssertFalse(app.buttons["Sync pending changes"].exists)
    ...
    func testMeetingNotesPersistAndRecordingRequiresAwareness() throws {
        ...
        XCTAssertFalse(app.staticTexts["Ready when you are"].exists)
        ...
        XCTAssertTrue(app.buttons["prepareMeetingRecording"].exists)
        XCTAssertFalse(app.buttons["startMeetingRecording"].exists)
        activate(app.buttons["prepareMeetingRecording"])
    ...
    private func launchNativeEditor() throws -> XCUIElement {
        guard let origin = ProcessInfo.processInfo.environment["ENCHIRIDION_EDITOR_TEST_ORIGIN"],
            ...
            throw XCTSkip("Set ENCHIRIDION_EDITOR_TEST_ORIGIN to the running website fixture to test the native editor.")
        }
        app.terminate()
        app.launchEnvironment["ENCHIRIDION_EDITOR_TEST_ORIGIN"] = origin
        app.launchArguments = ["--ui-testing", "--reset-test-data", "--native-editor"]
        app.launchEnvironment["ENCHIRIDION_NATIVE_TEST_DOCUMENT_ID"] = "native-test:" + UUID().uuidString
        app.launch()
        openDailyNoteIfNeeded()
