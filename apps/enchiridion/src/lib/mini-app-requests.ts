...
export function buildMiniAppGenerationPrompt(input: {
	...
	const sections = [
		...
		"- If the app needs private persistent state that host APIs do not provide, declare the smallest isolated binding set in bindings and use only env.<DECLARED_BINDING>. Autonomous deploy will stop as a pending provisioning request until those resources exist.",
