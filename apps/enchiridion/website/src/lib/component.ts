import {
	...
	type Playback,
	safeURL,
} from "@enchiridion/documents/note";
export type {
	DrawingDocument,
	DrawingElement,
	DrawingInk,
...
export const defaultComponent = (kind: ComponentKind): NoteComponent => {
...
export const webURL = (value: string | undefined): string | undefined => {
...
export const elementBounds = (element: DrawingElement) => {
...
export const elementPath = (element: DrawingElement): string => {
	const first = element.points[0];
	if (!first) return "";
	if (element.kind === "pen") {
		return (
			`M ${first.x} ${first.y} ` +
			(element.points.length === 1 ? `l .1 0` : element.points
				.slice(1)
	...
	return (
		...
		[-0.45, 0.45]
			.map(
				(offset) =>
					`M ${last.x} ${last.y} L ${last.x - Math.cos(angle + offset) * 18} ${
						last.y - Math.sin(angle + offset) * 18
					}`,
			)
			.join(" ")
	);
...
export const drawingSVG = (document: DrawingDocument): string => {
	...
	const elements = document.elements
		.map((element) => {
			...
			const style = `stroke="${
				inks[element.ink] ?? inks.graphite
			}" stroke-width="${
				Number(element.lineWidth) || 3
			}" fill="none" stroke-linecap="round" stroke-linejoin="round"`;
			if (element.kind === "text" && first) {
				return `<text x="${Number(first.x)}" y="${
					Number(first.y) + 21
				}" font-family="system-ui,sans-serif" font-size="21" font-weight="500" fill="${
					inks[element.ink] ?? inks.graphite
				}">${escape(element.text)}</text>`;
			}
			if (element.kind === "rectangle") {
				return `<rect x="${bounds.x}" y="${bounds.y}" width="${bounds.width}" height="${bounds.height}" rx="10" ${style}/>`;
			}
			if (element.kind === "ellipse") {
				return `<ellipse cx="${bounds.x + bounds.width / 2}" cy="${
					bounds.y + bounds.height / 2
				}" rx="${bounds.width / 2}" ry="${bounds.height / 2}" ${style}/>`;
			}
...
export const svgImage = (svg: string | undefined): string | undefined => {
	return svg
