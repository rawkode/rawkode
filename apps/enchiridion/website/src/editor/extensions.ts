...
import {
	linkAttributesSchema,
	orderedListAttributesSchema,
	parseComponent,
	parseEntity,
	safeURL,
	textStyleAttributesSchema,
} from "@enchiridion/documents/note";

const validatedHTMLAttributes = (
	...
	for (const [name, attribute] of Object.entries(attributes)) {
		...
		normalized[name] = {
			...
			parseHTML(element: HTMLElement) {
				const value = attribute.parseHTML
					? attribute.parseHTML(element)
					: element.getAttribute(name);
				const result = schema.safeParse(
					emptyToNull && value === "" ? null : value,
				);
				return result.success
					? result.data ?? attribute.default ?? null
...
const withSafeStyleParsing = (extension: Extension): Extension => {
	return extension.extend({
		addGlobalAttributes() {
			return (this.parent?.() ?? []).map((group) => ({
				...group,
				attributes: validatedHTMLAttributes(
					group.attributes,
					textStyleAttributesSchema.shape,
					true,
				),
			}));
		},
...
const SafeLink = Link.extend({
	addAttributes() {
		return validatedHTMLAttributes(
			this.parent?.() ?? {},
			linkAttributesSchema.shape,
		);
	},
});
...
const SafeOrderedList = OrderedList.extend({
	addAttributes() {
		return validatedHTMLAttributes(
			this.parent?.() ?? {},
			orderedListAttributesSchema.shape,
		);
	},
});
...
export const canConvertSelectionToCode = (
...
const SafeCodeBlock = CodeBlock.extend({
	addCommands() {
		return {
			setCodeBlock: (attrs) => ({ state, commands }) =>
				canConvertSelectionToCode(state) &&
				commands.setNode(this.name, attrs),
			toggleCodeBlock: (attrs) => ({ state, commands }) =>
				canConvertSelectionToCode(state) &&
...
export const applyBlockStyle = (editor: Editor, style: BlockStyle): boolean => {
