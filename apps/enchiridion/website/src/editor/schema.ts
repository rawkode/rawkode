...
import { extensions, validateExtension as validateDiagram } from './extensions/registry';
...
export function editorExtensions(options: Pick<DailyEditorOptions, 'onDiagramEdit' | 'loadPreview'>): Extensions {
  const Diagram = Node.create({
    ...
    addAttributes() {
      return { id: { default: null }, kind: { default: 'd2' }, version: { default: 1 }, source: { default: '' } };
    },
    parseHTML() {
      return [{ tag: 'figure[data-diagram]', getAttrs(element) {
        const attrs = { id: crypto.randomUUID(), kind: element.getAttribute('data-diagram'), version: Number(element.getAttribute('data-version') || 1), source: element.getAttribute('data-source') ?? '' };
        try { validateDiagram(attrs.kind, attrs.source, attrs.id, attrs.version); return attrs; } catch { return false; }
      } }];
    },
    renderHTML({ node, HTMLAttributes }) {
      return ['figure', mergeAttributes(HTMLAttributes, {
        'data-diagram': node.attrs.kind, 'data-version': node.attrs.version, 'data-id': node.attrs.id, 'data-source': node.attrs.source,
    ...
    addNodeView() {
      return ({ node: initial }) => {
        ...
        button.addEventListener('click', event => {
          ...
          options.onDiagramEdit({ id: node.attrs.id, kind: node.attrs.kind, version: node.attrs.version, source: node.attrs.source });
        });
        ...
        const refresh = () => {
          ...
          label.textContent = extension?.label || `${node.attrs.kind} v${node.attrs.version} (extension unavailable, source preserved)`;
          ...
          options.loadPreview?.(node.attrs.kind, node.attrs.source).then(url => {
            if (version === generation && (url?.startsWith('data:image/png;base64,') || url?.startsWith('data:image/svg+xml;base64,'))) {
              image.src = url;
              image.hidden = false;
            }
        ...
        return {
          ...
          ignoreMutation: () => true,
          destroy: () => { generation++; document.removeEventListener('document-extension-preview', previewUpdated); },
        };
      };
    },
  });
