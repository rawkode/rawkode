...
export function createDailyEditor(options: DailyEditorOptions): DailyEditorHandle {
  ...
  const editor = new Editor({
    ...
    editorProps: { attributes: { class: 'daily-prose', role: 'textbox', 'aria-label': 'Daily note', 'aria-multiline': 'true', spellcheck: 'true' } },
  ...
  return {
    ...
    insertDiagram(kind, source = '', version = 1) {
      ...
      editor.chain().focus().insertContent([{ type: 'diagram', attrs: { id, kind, version, source } }, { type: 'paragraph' }]).run();
      return id;
    },
