...
export function createDailyEditor(
	options: DailyEditorOptions,
): DailyEditorHandle {
	if (!/^\d{4}-\d{2}-\d{2}$/.test(options.day))
		throw new Error('Invalid daily note date.');
	...
	try {
		if (options.snapshot) {
			...
			updateLoroToPmState(
				doc as LoroDocType,
				new Map(),
				EditorState.create({ schema, doc: initial }),
			);
	...
	const collaboration = Extension.create({
		...
		addProseMirrorPlugins: () => [
			LoroSyncPlugin({ doc: doc as LoroDocType }),
	...
	const emitSelection = () => {
		...
		if (!selection.empty) {
			try {
				...
				/* A detached view has no selection geometry. */
			}
		}
		options.onSelection({
			empty: selection.empty,
			rect,
			formats: ['bold', 'italic', 'underline', 'strike', 'code', 'link'].filter(
	...
	const editor = new Editor({
		...
		extensions: [...extensions, collaboration],
	...
	const unsubscribe = doc.subscribe((event) => {
		...
		queueMicrotask(() => {
			...
			if (active && initialized)
				options.onChange(doc.export({ mode: 'snapshot' }));
		});
	});
	...
	return {
		...
		prepareForTransition() {
			...
			if (editor.view.composing)
				throw new Error(
					'Finish the current text composition before changing days or quitting.',
				);
			flushNativeInput();
			editor.setEditable(false, false);
		},
		insertDiagram(kind, source = '', version = 1) {
			if (!active || !initialized) throw new Error('The editor is not ready.');
			const id = crypto.randomUUID();
			validateDiagram(kind, source, id, version);
			editor
				.chain()
				.focus()
				.insertContent([
					{ type: 'diagram', attrs: { id, kind, version, source } },
					{ type: 'paragraph' },
				])
				.run();
			return id;
		...
		updateDiagram(id, expectedSource, source) {
			...
			if (position === undefined)
				throw new Error(
					'This diagram was removed. Your changes have not overwritten the note.',
				);
			const node = editor.state.doc.nodeAt(position)!;
			if (node.attrs.source !== expectedSource)
				throw new Error(
					'This diagram changed while it was open. Reopen it before editing.',
				);
			validateDiagram(node.attrs.kind, source, id, node.attrs.version);
