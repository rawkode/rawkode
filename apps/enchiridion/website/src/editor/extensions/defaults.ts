...
extensions.register({
	...
	version: 1,
	label: 'D2 diagram',
	description: 'Turn a few lines into a diagram',
	initialSource: 'idea: An idea\nplan: A plan\nidea -> plan',
	validate(source) {
		if (source.length > 100_000) throw new Error('D2 source is too large');
	},
	load: () => import('./d2'),
});
extensions.register({
	...
	validate(source) {
		...
		if (
			...
			throw new Error('Embedded images and web content are not supported yet');
	},
	load: () => import('./drawing'),
});
