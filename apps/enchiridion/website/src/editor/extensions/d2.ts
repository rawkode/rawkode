...
export async function mount(
	...
	input.value = source;
	input.setAttribute('aria-label', 'D2 source');
	input.spellcheck = false;
	const preview = document.createElement('img');
	preview.alt = 'Diagram preview';
	const button = document.createElement('button');
	button.textContent = 'Preview diagram';
	const status = document.createElement('p');
	status.setAttribute('role', 'status');
	element.className = 'd2-workspace';
	element.append(input, button, status, preview);
	let renderedSource = '',
		rendered = '',
		active = true;
	const render = async () => {
		...
		try {
			...
			const compiled = await engine.compile(current, {
				layout: 'dagre',
				pad: 24,
			});
			...
			renderedSource = current;
			preview.src = rendered;
			status.textContent = 'Preview ready';
		} catch {
			if (active)
				status.textContent =
					'Could not render. Check the D2 source; it is still editable.';
		} finally {
	...
	return {
		async read() {
			return {
				source: input.value,
				preview: renderedSource === input.value ? rendered : undefined,
			};
		},
		destroy() {
			active = false;
