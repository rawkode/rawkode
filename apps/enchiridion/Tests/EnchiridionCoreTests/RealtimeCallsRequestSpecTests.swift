...
struct RealtimeCallsRequestSpecTests {
  ...
  func bundledBridgeHasStrictCSPAndNoCredentialMaterial() throws {
    ...
    #expect(html.contains("const operationEpoch = message.inputEpoch"))
    #expect(html.contains("message.inputEpoch <= inputEpoch"))
    #expect(html.contains("inputEpoch = Number.MAX_SAFE_INTEGER"))
    #expect(html.contains("inputQueue = operation.catch(() => {})"))
    #expect(html.contains("if (!message.enabled) {\n        track.enabled = false;\n        return;"))
    #expect(html.contains("if (!message.enabled && microphone) canonicalAudioTrack(microphone).enabled = false;"))
    ...
    #expect(html.contains("replacementSender = senders[0]"))
    #expect(html.contains("replacementSender.replaceTrack(replacementTrack)"))
    #expect(html.contains("await replacementSender.replaceTrack(replacementTrack)"))
    #expect(html.contains("replacementSender.track !== replacementTrack"))
    #expect(html.contains("async function restoreInputSender"))
    #expect(html.contains("await restoreInputSender(replacementSender, oldTrack, replacementTrack, sessionPeer)"))
    #expect(html.contains("await sender.replaceTrack(oldTrack)"))
    #expect(html.contains("replacementSender.track === replacementTrack"))
