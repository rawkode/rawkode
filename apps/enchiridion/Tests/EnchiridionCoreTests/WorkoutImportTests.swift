...
final class WorkoutImportTests: XCTestCase {
  func testStrengthImportCreatesDeterministicGraphAndReplayPreservesEdits() async throws {
    ...
    guard case .imported(let rootID, _) = imported else { return XCTFail("Expected import") }
    ...
    guard case .duplicate(let replayedID, _) = replay else { return XCTFail("Expected duplicate") }
  ...
  func testCardioCapturesAggregatesAndRejectsMalformedPayload() async throws {
    ...
    let cardio = WorkoutCaptureEnvelope(
      eventID: "10000000-0000-0000-0000-000000000001", startedAt: start,
      completedAt: start.addingTimeInterval(600), activity: .outdoorRun, status: .complete,
      durationSeconds: 600,
      payload: .cardio(
        splits: [
          .init(ordinal: 1, distanceMeters: 1_000, durationSeconds: 300),
          .init(ordinal: 2, distanceMeters: 1_000, durationSeconds: 300),
        ], distanceMeters: 2_000, elevationMeters: 32, averageSpeedMetersPerSecond: 3.33,
        averagePaceSecondsPerKilometre: 300))
    guard
      case .imported(let rootID, _) = try await repository.importWorkout(cardio, registry: registry)
    else { return XCTFail("Expected cardio import") }
    ...
    let invalid = WorkoutCaptureEnvelope(
      eventID: "10000000-0000-0000-0000-000000000002", startedAt: start,
      completedAt: start.addingTimeInterval(10), activity: .outdoorRun, status: .complete,
      durationSeconds: 10,
      payload: .cardio(
        splits: [.init(ordinal: 1, distanceMeters: 0, durationSeconds: 1)], distanceMeters: .nan,
        elevationMeters: nil, averageSpeedMetersPerSecond: nil, averagePaceSecondsPerKilometre: nil)
    )
    await XCTAssertThrowsErrorAsync(try await repository.importWorkout(invalid, registry: registry))
    { error in
  ...
  func testSameEventDifferentPayloadIsDurablyQuarantined() async throws {
    ...
    let changed = WorkoutCaptureEnvelope(
      eventID: first.eventID, startedAt: first.startedAt, completedAt: first.completedAt,
      activity: first.activity, status: .complete, durationSeconds: first.durationSeconds,
      payload: .strength(exercises: [
        .init(
          ordinal: 1, name: "Squat", sets: [.init(ordinal: 1, repetitions: 9, loadKilograms: 100)])
      ]))
  ...
  func testSameEnvelopeIsIsolatedToEachVaultDatabase() async throws {
    ...
    let envelope = strengthEnvelope()
    guard
      case .imported(let firstID, _) = try await first.importWorkout(envelope, registry: registry),
      case .imported(let secondID, _) = try await second.importWorkout(envelope, registry: registry)
  ...
  func testCompiledDeclarationsStayOutOfCloudSchemaWhileImportedPagesAreDirty() async throws {
    ...
    let registry = try ModuleRegistry(manifests: [WorkoutModule.manifest])
    guard
      case .imported(let rootID, _) = try await repository.importWorkout(
        strengthEnvelope(), registry: registry)
    else { return XCTFail("Expected import") }
    ...
    XCTAssertFalse(
      dirtyRelations.contains { $0.definition.id.rawValue.hasPrefix(WorkoutModule.id.rawValue) })
    let cloudDirty: Int = try await repository.database.read { db in
      try Int.fetchOne(
        db, sql: "SELECT cloud_dirty FROM pages WHERE id = ?", arguments: [rootID.rawValue]) ?? 0
    }
  ...
  func testDeterministicRootRecoversWhenReceiptWasLost() async throws {
    ...
    try await repository.database.write { db in
      try db.execute(
        sql: "DELETE FROM workout_import_receipts WHERE module_id = ? AND event_id = ?",
        arguments: [envelope.moduleID, envelope.eventID])
    }
    guard
      case .duplicate(let recoveredID, _) = try await repository.importWorkout(
        envelope, registry: registry)
    else { return XCTFail("Expected root-provenance recovery") }
  ...
  private func strengthEnvelope() -> WorkoutCaptureEnvelope {
    let start = Date(timeIntervalSince1970: 1_000)
    return .init(
      eventID: "00000000-0000-0000-0000-000000000001", startedAt: start,
      completedAt: start.addingTimeInterval(120), activity: .strengthTraining, status: .complete,
      durationSeconds: 120,
      payload: .strength(exercises: [
        .init(
          ...
          sets: [
            ...
            .init(
              ordinal: 2, repetitions: 5, loadKilograms: 100,
  ...
  private func makeRepository() throws -> LibraryRepository {
    let directory = FileManager.default.temporaryDirectory.appendingPathComponent(
      "WorkoutImportTests-\(UUID().uuidString)")
...
private func XCTAssertThrowsErrorAsync<T>(
  _ expression: @autoclosure () async throws -> T, _ handler: (Error) -> Void = { _ in }
) async {
  do {
