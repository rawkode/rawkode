...
Set `MULTIPASS_SIGNING_IDENTITY` to a configured identity for a stable development signature. The default ad-hoc app is not notarized, and macOS keys Input Monitoring, Local Network and keychain access to the code signature: after every ad-hoc rebuild those grants silently stop matching while System Settings still shows them enabled. The Nix darwin module therefore signs the installed app with a self-signed certificate it generates once per host (see the `rawkOS.darwin.codeSigning` module, shared with other locally built apps), so the signature, and every grant tied to it, survives rebuilds. The app asks for Input Monitoring itself when switching is on, so it appears in that list even on a Mac the mouse is currently away from. With any other ad-hoc build, turn Multipass off and on again under Input Monitoring after rebuilding. Enabling switching now probes the mouse and reports in the activity log when the OS blocks access. Distribution beyond development needs normal signing/notarization. Do not run the app as root.
