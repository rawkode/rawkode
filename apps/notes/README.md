...
Local query blocks support table, list, and board views over SQLite-backed documents, daily notes, entities, supertags, supertag collections, and entity backlinks, with `SELECT *` or explicit field projections plus optional `WHERE`, `ORDER BY`, and `LIMIT` clauses. Supertag collection rows expose entity properties as queryable columns. Property values written as `[[Entity Name]]` are stored as entity references and expose a matching `<property>_entity_id` query column. Cloudflare sync/storage and a broader query language are still roadmap features.
