// access-auth.test.ts — real crypto round-trip tests for
// `verifyAccessRequest` (./access-auth.ts). No step here is mocked to
// "always succeed": every test signs a genuine JWT with a genuine RSA
// keypair (via `jose`'s own `generateKeyPair`/`SignJWT`), serves it through
// a FAKE JWKS HTTP response (`options.fetchImpl`, ./access-auth.ts's
...
// the module-level JWKS cache in access-auth.ts (one `RemoteJWKSet` per
// team domain, see that file's header) never lets one test's fake JWKS
