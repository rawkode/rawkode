...
export const validOwnerVaultFloorSyncCommand = (
  value: OwnerVaultFloorSyncCommand,
): boolean =>
  identity.test(value.ownerID) &&
  identity.test(value.vaultID) &&
  value.ownerID !== value.vaultID &&
...
export const floorSyncDigest = (command: Omit<OwnerVaultFloorSyncCommand, "floorSyncDigest">): string =>
  sha256Hex(new TextEncoder().encode(JSON.stringify({
    credentialEpoch: command.credentialEpoch,
    controlEpoch: command.controlEpoch,
    generationEpoch: command.generationEpoch,
...
export const sameOwnerVaultFloorSyncAck = (
  ...
  left.generationEpoch === right.generationEpoch && left.operationID === right.operationID &&
  left.credentialEpoch === right.credentialEpoch && left.routingEpoch === right.routingEpoch &&
  left.controlEpoch === right.controlEpoch && left.floorSyncDigest === right.floorSyncDigest &&
  durableReceipt.test(right.durableReceipt);
...
const decodeFloorSyncAck = (value: unknown): OwnerVaultFloorSyncAck | undefined => {
  if (value === null || typeof value !== "object" || Array.isArray(value)) return undefined;
  const source = Object.fromEntries(Object.entries(value));
  const keys = ["ownerID", "vaultID", "generationEpoch", "operationID", "credentialEpoch", "routingEpoch", "controlEpoch", "floorSyncDigest", "durableReceipt"];
  if (Object.keys(source).length !== keys.length || keys.some((key) => !Object.hasOwn(source, key))) return undefined;
...
export interface OwnerVaultFloorSyncClient {
  readonly syncFloors: (command: OwnerVaultFloorSyncCommand, capability: SignedCapability) => Effect.Effect<OwnerVaultFloorSyncAck, OwnerVaultInitializationError>;
}
...
export const makeOwnerVaultFloorSyncClient = (namespace: DurableObjectNamespaceNative): OwnerVaultFloorSyncClient => ({
  syncFloors: (command, capability) => Effect.tryPromise({
    try: async () => {
      if (!validOwnerVaultFloorSyncCommand(command)) throw new OwnerVaultInitializationError({ reason: "invalid_command" });
      const stub = namespace.get(namespace.idFromName(ownerVaultObjectName(command)));
      const payload = new TextEncoder().encode(JSON.stringify({ capability: capability.value, command }));
      const requestBody = new Uint8Array(payload.byteLength);
      requestBody.set(payload);
      const response = await stub.fetch(new Request(`https://owner-vault.invalid${ownerVaultFloorSyncPath}`, {
        method: "POST", headers: { "content-type": "application/json" }, body: requestBody.buffer,
      }));
      if (response.status !== 200) throw new OwnerVaultInitializationError({ reason: "unavailable" });
      const ack = decodeFloorSyncAck(JSON.parse(await response.text()));
      if (ack === undefined || !sameOwnerVaultFloorSyncAck(command, ack)) throw new OwnerVaultInitializationError({ reason: "ack_mismatch" });
      return ack;
    },
    catch: (cause) => cause instanceof OwnerVaultInitializationError ? cause : new OwnerVaultInitializationError({ reason: "unavailable" }),
  }),
...
export const signOwnerVaultFloorSync = (
  ...
  if (!validOwnerVaultFloorSyncCommand(command)) return Effect.fail(new OwnerVaultInitializationError({ reason: "invalid_command" }));
  ...
  return signer.sign({
    ...
    method: CapabilityMethod.POST, path: ownerVaultFloorSyncPath, canonicalQuery: "", bodySHA256,
    ownerID: command.ownerID, vaultID: command.vaultID, floorSyncDigest: command.floorSyncDigest,
    credentialEpoch: command.credentialEpoch, controlEpoch: command.controlEpoch,
    generationEpoch: command.generationEpoch,
    routingEpoch: command.routingEpoch, operationID: command.operationID, jti, ttlSeconds: 60,
  }, nowSeconds).pipe(Effect.mapError(() => new OwnerVaultInitializationError({ reason: "unavailable" })));
