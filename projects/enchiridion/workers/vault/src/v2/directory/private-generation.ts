...
export const DirectoryPrivateGenerationService =
  Context.GenericTag<DirectoryPrivateGenerationService>(
...
export const DirectoryOwnerVaultPrivateGeneration =
  Context.GenericTag<DirectoryOwnerVaultPrivateGeneration>(
...
const authoritativeRoot = (
  ...
  const matches = Object.values(state.bindings).filter(
    (candidate) =>
      candidate.ownerID.value === target.ownerID && candidate.vaultID.value === target.vaultID,
  );
  return matches.length === 1 ? matches[0] : undefined;
};
  ...
  Effect.gen(function* () {
    ...
    const prepare = (
      ...
      return random.identifier("owner-vault-initialization").pipe(
        Effect.mapError(
          () => new DirectoryPrivateGenerationError({ reason: "initialization_unavailable" }),
        ),
        Effect.flatMap((generatedInitializationOperation) =>
          repository
            .transact<Reservation>((state) =>
              Effect.suspend(() => {
                const existing = state.privateGenerations[operation];
                if (existing !== undefined) {
                  if (
                    existing.ownerID !== source.ownerID ||
                    existing.vaultID !== source.vaultID ||
                    existing.generationEpoch <= source.generationEpoch
                  )
                    return Effect.fail(
                      transactionFailure(
                        new DirectoryPrivateGenerationError({ reason: "operation_conflict" }),
                      ),
                    );
                  return Effect.succeed([
                    {
                      _tag:
                        existing.initialization.durableReceipt === undefined
                          ? "initialize"
                          : "reconcile",
                      target: existing,
                    } as const,
                ...
                const generationEpoch =
                  resolved === undefined ? undefined : targetGeneration(state, resolved);
                if (
                  Object.keys(state.privateGenerations).length >= maximumDirectoryPrivateGenerations
                )
                  return Effect.fail(
                    transactionFailure(
                      new DirectoryPrivateGenerationError({ reason: "root_unavailable" }),
                ...
                return Effect.succeed([
                  ...
                  {
                    ...state,
                    privateGenerations: { ...state.privateGenerations, [operation]: target },
                  },
                ] as const);
              }),
            )
                ...
                (reservation._tag === "initialize"
                  ? signOwnerVaultInitialization(
                      ...
                      Effect.flatMap((capability) =>
                        ownerVault.initialization.ensureInitialized(
                          reservation.target.initialization,
                          capability,
                        ),
                      ),
                      Effect.mapError(
                        () =>
                          new DirectoryPrivateGenerationError({
                            reason: "initialization_unavailable",
                          }),
                      ),
                      Effect.flatMap((ack) =>
                        persistInitializationAck(repository, reservation.target, ack),
                      ),
                    )
    ...
    .transact((state) =>
      Effect.suspend(() => {
        ...
        if (
          ...
          return Effect.fail(
            transactionFailure(new DirectoryPrivateGenerationError({ reason: "root_unavailable" })),
          );
        ...
        return Effect.succeed([
          ...
          {
            ...state,
            privateGenerations: { ...state.privateGenerations, [updated.operationID]: updated },
          },
        ] as const);
      }),
    )
...
type Reconciliation =
  ...
  | {
      readonly _tag: "sync";
      readonly target: DirectoryPrivateGeneration;
      readonly command: OwnerVaultFloorSyncCommand;
    };
...
const reconcile = (
  ...
  return random.identifier("owner-vault-floor-sync").pipe(
    ...
    Effect.flatMap((generatedOperation) =>
      repository
        .transact<Reconciliation>((state) =>
          Effect.suspend(() => {
            const current = state.privateGenerations[operation];
            const authoritative =
              current === undefined ? undefined : authoritativeRoot(state, current);
            if (
              current === undefined ||
            ...
            if (currentFloors) {
              ...
              return Effect.succeed<readonly [Reconciliation, DirectoryState]>([
                ...
                {
                  ...state,
                  privateGenerations: { ...state.privateGenerations, [operation]: ready },
                },
              ] as const);
            }
            ...
            const command =
              existing !== undefined && existing.durableReceipt === undefined
                ...
                : (() => {
                    ...
                    return {
                      ...unsigned,
                      floorSyncDigest: floorSyncDigest(unsigned),
                    } satisfies OwnerVaultFloorSyncCommand;
                  })();
            const updated = existing !== command ? { ...current, floorSync: command } : current;
            return Effect.succeed<readonly [Reconciliation, DirectoryState]>([
              { _tag: "sync" as const, target: updated, command },
              {
                ...state,
                privateGenerations: { ...state.privateGenerations, [operation]: updated },
              },
            ] as const);
          }),
        )
        .pipe(
          ...
          Effect.flatMap((next) => {
            ...
            return signOwnerVaultFloorSync(
              controls.signer,
              next.command,
              next.command.operationID,
              nowSeconds,
            ).pipe(
              Effect.flatMap((capability) =>
                ownerVault.floors.syncFloors(next.command, capability),
              ),
              Effect.mapError(
                () => new DirectoryPrivateGenerationError({ reason: "sync_unavailable" }),
              ),
              Effect.flatMap((ack) =>
                repository
                  .transact((state) =>
                    Effect.suspend(() => {
                      ...
                      if (
                        current === undefined ||
                        current.floorSync === undefined ||
                        current.floorSync.durableReceipt !== undefined ||
