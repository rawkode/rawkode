...
import {
  ...
  type OwnerVaultDomainTransactionError,
...
const object = (value: unknown): Readonly<Record<string, unknown>> | undefined =>
  value !== null && typeof value === "object" && !Array.isArray(value)
    ...
    : undefined;
const payload = (value: object): Readonly<Record<string, unknown>> => Object.fromEntries(Object.entries(value));
...
const domainFailure = (error: BlobOperationError): OwnerVaultDomainTransactionError => ({
  ...
  reason: error.reason === "generation_stale"
...
const blobFailureFromTransaction = (error: OwnerVaultStorageTransactionFailure | OwnerVaultStorageRepositoryError): BlobOperationError =>
  new BlobError({
    reason: error._tag === "OwnerVaultDomainTransactionError"
...
const storageAddress = (category: OwnerVaultStorageAddress["category"], identifier?: string): OwnerVaultStorageAddress =>
...
const decodeLease = (record: OwnerVaultStorageRecord | undefined): Lease | undefined => {
  const source = record === undefined ? undefined : object(record.payload);
  const keys = ["authEpoch", "credentialEpoch", "deviceID", "expiresAtSeconds", "finalKey", "operationID", "phase", "requestID", "securityFloor", "sha256", "size", "stageKey"] as const;
  if (source === undefined || !exact(source, keys) ||
    (source.phase !== "RESERVED" && source.phase !== "PUBLISHED" && source.phase !== "CLEANUP") ||
    ![source.requestID, source.operationID, source.deviceID, source.stageKey, source.finalKey].every((value) => typeof value === "string" && value.length > 0 && value.length <= 1024) ||
    typeof source.sha256 !== "string" || !hash.test(source.sha256) || !integer(source.size) || !integer(source.authEpoch, 1) || !integer(source.credentialEpoch, 1) || !integer(source.securityFloor) || !integer(source.expiresAtSeconds))
    return undefined;
  return { phase: source.phase, requestID: source.requestID as string, operationID: source.operationID as string, deviceID: source.deviceID as string, stageKey: source.stageKey as string, finalKey: source.finalKey as string, sha256: source.sha256, size: source.size, authEpoch: source.authEpoch, credentialEpoch: source.credentialEpoch, securityFloor: source.securityFloor, expiresAtSeconds: source.expiresAtSeconds };
};
...
const decodePurge = (record: OwnerVaultStorageRecord | undefined): Purge | undefined => {
  ...
  return source !== undefined && exact(source, ["leaseID", "objectKey", "startedAtSeconds"]) &&
    typeof source.objectKey === "string" && typeof source.leaseID === "string" && safeID.test(source.leaseID) && integer(source.startedAtSeconds)
    ? { objectKey: source.objectKey, leaseID: source.leaseID, startedAtSeconds: source.startedAtSeconds }
    : undefined;
};
const decodeReceipt = (record: OwnerVaultStorageRecord | undefined): StoredReceipt | undefined => {
  const source = record === undefined ? undefined : object(record.payload);
  if (source === undefined || !exact(source, ["fingerprint", "kind", "response"]) || typeof source.fingerprint !== "string") return undefined;
...
const stageFingerprint = (command: BlobStageCommand): string => [command.operationID.value, command.requestID.value, command.stageRandom, command.path, command.sha256, String(command.size), command.deviceID, String(command.authEpoch), String(command.credentialEpoch)].join("\u0000");
const deleteFingerprint = (authorization: BlobAuthorization, sha256: string, requestID: string): string => [authorization.deviceID, String(authorization.authEpoch), String(authorization.credentialEpoch), sha256, requestID].join("\u0000");
const sameScope = (left: BlobScope, right: BlobScope): boolean => left.ownerID.value === right.ownerID.value && left.vaultID.value === right.vaultID.value && left.generationEpoch === right.generationEpoch;
...
export interface OwnerVaultBlobLifecycleRepository extends BlobStagingRepository {
  readonly deleteBySHA256: (command: BlobDeleteCommand) => Effect.Effect<{ readonly sha256: string; readonly status: "APPLIED" }, BlobOperationError>;
...
export const makeOwnerVaultBlobStagingRepository = (
  ...
  const validConfig = Number.isSafeInteger(config.deleteGraceSeconds) && config.deleteGraceSeconds >= 1 &&
    config.limits.maximumActiveLeasesPerVault >= 1 && config.limits.maximumActiveLeasesPerVault <= maximumTrackedLeases;
  const transact = <A>(operation: (tx: OwnerVaultTx) => Effect.Effect<A, BlobTransactionFailure>): Effect.Effect<A, BlobOperationError> =>
    validConfig
    ...
    Effect.all([tx.get(floorsAddress), tx.get(storageAddress("device", authorization.deviceID))]).pipe(
      Effect.flatMap(([floorRecord, deviceRecord]) => {
        ...
        return floors === undefined || device === undefined ||
          ...
          device.authEpoch !== authorization.authEpoch || device.credentialEpoch !== authorization.credentialEpoch ||
          !integer(device.securityFloor) || device.securityFloor !== floors.securityFloor
  ...
  const preflightReceipt: BlobStagingRepository["preflightReceipt"] = (command, fingerprint) =>
    transact((tx) => currentAuthority(tx, stageAuthorization(command)).pipe(Effect.zipRight(tx.get(receiptAddress(command.requestID.value))), Effect.flatMap((row) => {
  ...
  const stageImmutable: BlobStagingRepository["stageImmutable"] = (stageKey, body, nowSeconds) =>
    ...
    transact((tx) => {
      ...
      return readLease(tx, operationID).pipe(Effect.flatMap((lease) =>
        lease === undefined || lease.stageKey !== stageKey || lease.phase !== "RESERVED" || lease.expiresAtSeconds <= nowSeconds
      ...
      config.r2.putIfAbsent(stageKey, new Uint8Array(body)).pipe(
        Effect.catchAll((error) => {
          ...
          return config.r2.read(stageKey).pipe(
            Effect.flatMap((stored) => stored.bytes.byteLength === body.byteLength && stored.bytes.every((value, index) => value === body[index]) ? Effect.void : failed("stage_conflict")),
            Effect.mapError(() => new BlobError({ reason: "stage_conflict" })),
          );
        }),
      ),
    ));
  const publishImmutable: BlobStagingRepository["publishImmutable"] = (stageKey, finalKey, expectedHash, size, nowSeconds) =>
    // R2 read/put is intentionally between lease-state transactions.
    config.r2.read(stageKey).pipe(
      Effect.mapError(() => new BlobError({ reason: "publish_failed" })),
      Effect.flatMap((stage) => stage.bytes.byteLength !== size ? failed("publish_failed") : config.r2.putIfAbsent(finalKey, stage.bytes).pipe(
        Effect.catchAll((error) => error.reason === "already_exists" ? config.r2.head(finalKey).pipe(Effect.flatMap((found) => found === undefined || found.size !== size || found.sha256Base64 !== hexToBase64(expectedHash) ? failed("publish_failed") : Effect.void), Effect.mapError(() => new BlobError({ reason: "publish_failed" }))) : failed("publish_failed")),
      )),
      Effect.zipRight(transact((tx) => readLease(tx, stageKey.split("/").at(-2) ?? "").pipe(Effect.flatMap((lease) => lease === undefined || lease.stageKey !== stageKey || lease.finalKey !== finalKey || lease.sha256 !== expectedHash || lease.size !== size || lease.expiresAtSeconds <= nowSeconds ? failed("stage_conflict") : tx.put(leaseAddress(lease.operationID), payload({ ...lease, phase: "PUBLISHED" })).pipe(Effect.mapError(() => new BlobError({ reason: "stage_conflict" }))))))),
    );
  const verifyFinal: BlobStagingRepository["verifyFinal"] = (stageKey, finalKey, expectedHash, size, nowSeconds) =>
    config.r2.head(finalKey).pipe(Effect.mapError(() => new BlobError({ reason: "final_verification_failed" })), Effect.flatMap((found) => found === undefined || found.size !== size || found.sha256Base64 !== hexToBase64(expectedHash) ? failed("final_verification_failed") : Effect.void));
  const discardStage: BlobStagingRepository["discardStage"] = (stageKey) => config.r2.deleteExact(stageKey).pipe(Effect.mapError(() => new BlobError({ reason: "stage_conflict" })));
  ...
  const commitStaged: BlobStagingRepository["commitStaged"] = (command, stageKey, objectKey, nowSeconds) =>
    transact((tx) => Effect.gen(function* () {
      const floors = yield* currentAuthority(tx, stageAuthorization(command));
      const tombstone = yield* tx.get(tombstoneAddress(command.sha256));
      ...
      if (lease === undefined || lease.phase !== "PUBLISHED" || lease.stageKey !== stageKey || lease.finalKey !== objectKey || lease.expiresAtSeconds <= nowSeconds || lease.securityFloor !== floors.securityFloor) return yield* failed<BlobStageExecution>("stage_conflict");
      ...
      transact((tx) =>
        readLease(tx, operationID).pipe(
          Effect.flatMap((lease) => {
            if (lease === undefined || lease.expiresAtSeconds > nowSeconds) return Effect.succeed(undefined);
  ...
  const deleteBySHA256: OwnerVaultBlobLifecycleRepository["deleteBySHA256"] = (command) =>
    transact<{ readonly sha256: string; readonly status: "APPLIED" }>((tx) => Effect.gen(function* () {
      if (!hash.test(command.sha256) || !safeID.test(command.requestID) || !integer(command.nowSeconds)) return yield* failed<{ readonly sha256: string; readonly status: "APPLIED" }>("invalid_blob");
      yield* currentAuthority(tx, command.authorization);
      const priorReceipt = yield* tx.get(receiptAddress(command.requestID));
      const fingerprint = deleteFingerprint(command.authorization, command.sha256, command.requestID);
      if (priorReceipt !== undefined) {
        const receipt = decodeReceipt(priorReceipt);
        return yield* (receipt?.kind === "blob-delete" && receipt.fingerprint === fingerprint ? Effect.succeed(receipt.response as { readonly sha256: string; readonly status: "APPLIED" }) : failed("replay_conflict"));
      }
      ...
      yield* tx.put(refAddress(command.sha256), { objectKey: reference.objectKey, size: reference.size, referenceCount: 0, activeLeaseCount: 0 });
  ...
  const reconcileDelete = (sha256: string, nowSeconds: number): Effect.Effect<number, BlobOperationError> =>
    transact((tx) =>
      Effect.all([tx.get(tombstoneAddress(sha256)), tx.get(purgeAddress(sha256)), tx.get(refAddress(sha256))]).pipe(
        Effect.flatMap(([tombstoneRow, purgeRow, referenceRow]) => {
          ...
          if (tombstone === undefined || purge === undefined || reference === undefined || tombstone.purgeAfterSeconds > nowSeconds || reference.referenceCount !== 0 || reference.activeLeaseCount !== 0)
            return Effect.succeed(undefined);
          return tx.put(purgeAddress(sha256), { objectKey: tombstone.objectKey, leaseID: purge.leaseID, startedAtSeconds: nowSeconds }).pipe(
  ...
  return Object.freeze({ preflightReceipt, reserveStage, stageImmutable, publishImmutable, verifyFinal, discardStage, releaseReservation, commitStaged, reconcileOrphans: reconcile, reconcile, deleteBySHA256 });
};
