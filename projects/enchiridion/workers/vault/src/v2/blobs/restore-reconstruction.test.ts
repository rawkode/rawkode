...
const valid = () =>
  ({
    metadata: [
      { sha256, requestID: "restore-request-0001", path: "notes/today", size: 3, objectKey },
    ],
    references: [
      { sha256, reference: { objectKey, size: 3, referenceCount: 1, activeLeaseCount: 0 } },
      {
        sha256: deletedSHA256,
        reference: { objectKey: deletedObjectKey, size: 4, referenceCount: 0, activeLeaseCount: 0 },
      },
    ],
    tombstones: [
      {
        sha256: deletedSHA256,
        tombstone: { objectKey: deletedObjectKey, deletedAtSeconds: 10, purgeAfterSeconds: 20 },
      },
    ],
  }) as const;
...
describe("OwnerVault blob restore reconstruction", () => {
  ...
  test("fails closed for malformed canonical reference/accounting invariants", () => {
    ...
    const result = reconstructOwnerVaultRestoredBlobInventory(scope, limits, {
      ...inventory,
      references: [
        { sha256, reference: { objectKey, size: 3, referenceCount: 1, activeLeaseCount: -1 } },
      ],
      tombstones: [],
    });
    expect(result).toEqual({
  ...
  test("fails closed for inconsistent reference, metadata, and tombstone lifecycle", () => {
    ...
    const result = reconstructOwnerVaultRestoredBlobInventory(scope, limits, {
      ...inventory,
      tombstones: [
        { sha256, tombstone: { objectKey, deletedAtSeconds: 10, purgeAfterSeconds: 20 } },
      ],
    });
    expect(result).toEqual({
      _tag: "OwnerVaultBlobRestoreReconstructionFailure",
  ...
  test("rejects active source leases and impossible restore limits instead of reviving lifecycle work", () => {
    ...
    const overflow = reconstructOwnerVaultRestoredBlobInventory(
      scope,
      { ...limits, maximumVaultBytes: 2 },
      valid(),
    );
  ...
  test("rejects a bounded but over-quota reconstructed reference total", () => {
    const inventory = valid();
    const result = reconstructOwnerVaultRestoredBlobInventory(
      scope,
      { ...limits, maximumBlobBytes: 3, maximumVaultBytes: 3, maximumOrphanBytes: 3 },
      {
        metadata: [
