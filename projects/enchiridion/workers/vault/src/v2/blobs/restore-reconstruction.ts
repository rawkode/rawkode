...
export type OwnerVaultBlobRestoreReconstructionResult =
  | {
      readonly _tag: "OwnerVaultBlobRestoreReconstructionSuccess";
      readonly value: OwnerVaultBlobRestoreReconstruction;
    }
  | {
...
const successful = (
  value: OwnerVaultBlobRestoreReconstruction,
): OwnerVaultBlobRestoreReconstructionResult => ({
  _tag: "OwnerVaultBlobRestoreReconstructionSuccess",
  value,
});
const integer = (value: number, minimum = 0): boolean =>
  Number.isSafeInteger(value) && value >= minimum;
const validLimits = (limits: BlobLimits): boolean =>
  integer(limits.maximumBlobBytes, 1) &&
  integer(limits.maximumVaultBytes, limits.maximumBlobBytes) &&
  integer(limits.maximumOrphanBytes) &&
  limits.maximumOrphanBytes <= limits.maximumVaultBytes &&
  integer(limits.maximumOrphanCount) &&
  integer(limits.maximumActiveLeasesPerVault, 1) &&
  integer(limits.maximumActiveLeasesPerFinal, 1) &&
  integer(limits.stageTTLSeconds, 1);
const validMetadata = (
  scope: BlobScope,
  limits: BlobLimits,
  value: OwnerVaultRestoredBlobMetadata,
): boolean =>
  hash.test(value.sha256) &&
  identifier.test(value.requestID) &&
  validBlobPath(value.path) &&
  integer(value.size) &&
  value.size <= limits.maximumBlobBytes &&
...
const uniqueSHA256 = <A extends { readonly sha256: string }>(values: readonly A[]): boolean =>
  values.every((value) => hash.test(value.sha256)) &&
  new Set(values.map((value) => value.sha256)).size === values.length;
const byHash = <A extends { readonly sha256: string }>(
  values: readonly A[],
): ReadonlyMap<string, A> => new Map(values.map((value) => [value.sha256, value]));
...
export const reconstructOwnerVaultRestoredBlobInventory = (
  ...
  const totalRows =
    inventory.metadata.length + inventory.references.length + inventory.tombstones.length;
  if (
    totalRows > maximumRestoredBlobRows ||
    !uniqueSHA256(inventory.metadata) ||
    !uniqueSHA256(inventory.references) ||
    !uniqueSHA256(inventory.tombstones) ||
  ...
  for (const item of inventory.references) {
    ...
    if (
      expectedObjectKey === undefined ||
      item.reference.objectKey !== expectedObjectKey ||
      item.reference.size > limits.maximumBlobBytes ||
      item.reference.activeLeaseCount !== 0
    )
      return failed("invalid_inventory");
    ...
    if (item.reference.referenceCount > 0) {
      if (
        ...
        metadataEntry.size !== item.reference.size ||
        metadataEntry.objectKey !== item.reference.objectKey
      )
  ...
  if (
    [...metadata.keys()].some(
      (sha256) => !inventory.references.some((reference) => reference.sha256 === sha256),
    ) ||
    [...tombstones.keys()].some(
      (sha256) => !inventory.references.some((reference) => reference.sha256 === sha256),
    )
  )
    return failed("lifecycle_conflict");
  ...
  return successful({
    ...
    tombstones: [...inventory.tombstones].sort((left, right) =>
