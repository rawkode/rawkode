...
describe("OwnerVault append proof", () => {
  test("binds an exact zero-based chain independently from catalog identity", () => {
    ...
    expect(ownerVaultAppendProofValidate({ ...scope, generationEpoch: 2 }, [entry(1), entry(2)])?.appendLogDigest).not.toBe(proof?.appendLogDigest);
    expect(ownerVaultAppendProofValidate(scope, [entry(1, "substituted"), entry(2)])?.appendLogDigest).not.toBe(proof?.appendLogDigest);
    const reordered = { source: "http" as const, payloadHash: "b".repeat(64), operationID: "operation-1", logSequence: 1, deviceID: "device-1", fingerprint: "a".repeat(64), payloadBase64: "opaque" };
    expect(ownerVaultAppendProofDn("0".repeat(64), reordered)).toBe(ownerVaultAppendProofDn("0".repeat(64), entry(1)));
    expect(ownerVaultAppendProofDn("0".repeat(64), { ...entry(1), logSequence: 2 })).not.toBe(ownerVaultAppendProofDn("0".repeat(64), entry(1)));
