...
export const isOwnerVaultCatalogCurrentPayload = (value: unknown): value is OwnerVaultCatalogCurrentPayload => {
  const source = plainRecord(value);
  return source !== undefined && exact(source, ["catalogRevision", "rootDigest"]) &&
    safeNonNegativeInteger(source.catalogRevision) && typeof source.rootDigest === "string" && base64.test(source.rootDigest);
};

const isEntry = (value: unknown, ordinal: number): value is OwnerVaultCatalogEntry => {
  const source = plainRecord(value);
  return source !== undefined && exact(source, ["ordinal", "key", "category", "bytes", "digest"]) &&
    source.ordinal === ordinal && typeof source.key === "string" && source.key.startsWith("v2.ov/") && source.key.length <= 512 &&
    typeof source.category === "string" && source.category.length > 0 && source.category.length <= 128 &&
    safeNonNegativeInteger(source.bytes) && source.bytes <= ownerVaultCatalogMaximumObjectBytes &&
    typeof source.digest === "string" && base64.test(source.digest);
};

export const isOwnerVaultCatalogPagePayload = (value: unknown): value is { readonly entries: readonly OwnerVaultCatalogEntry[]; readonly digest: string } => {
  const source = plainRecord(value);
  if (source === undefined || !exact(source, ["entries", "digest"]) || !Array.isArray(source.entries) || source.entries.length > ownerVaultCatalogMaximumPageEntries || typeof source.digest !== "string" || !base64.test(source.digest)) return false;
  const first = source.entries[0] === undefined ? 0 : plainRecord(source.entries[0])?.ordinal;
  if (!safeNonNegativeInteger(first) || !source.entries.every((entry, index) => isEntry(entry, first + index))) return false;
  const digest = ownerVaultCatalogDigest(source.entries);
  const bytes = ownerVaultCatalogCanonicalBytes({ entries: source.entries, digest: source.digest })?.byteLength;
  return digest === source.digest && bytes !== undefined && bytes <= ownerVaultCatalogMaximumPageBytes;
};

const isScope = (value: unknown): boolean => {
  const source = plainRecord(value);
  return source !== undefined && exact(source, ["ownerID", "vaultID", "generationEpoch", "namespaceState"]) &&
    typeof source.ownerID === "string" && source.ownerID.length > 0 &&
    typeof source.vaultID === "string" && source.vaultID.length > 0 &&
    safeNonNegativeInteger(source.generationEpoch) && source.generationEpoch >= 1 &&
    (source.namespaceState === "PRIVATE" || source.namespaceState === "ACTIVE");
};

const isDescriptor = (value: unknown, ordinal: number, catalogRevision: number): value is OwnerVaultCatalogPageDescriptor => {
  const source = plainRecord(value);
  const expectedPrefix = ownerVaultCatalogRevisionIdentifier(catalogRevision);
  return source !== undefined && exact(source, ["ordinal", "identifier", "count", "bytes", "digest"]) &&
    source.ordinal === ordinal && typeof source.identifier === "string" && expectedPrefix !== undefined && source.identifier.startsWith(`${expectedPrefix}-`) && pageIdentifier.test(source.identifier) &&
    safeNonNegativeInteger(source.count) && source.count > 0 && source.count <= ownerVaultCatalogMaximumPageEntries &&
    safeNonNegativeInteger(source.bytes) && source.bytes <= ownerVaultCatalogMaximumPageBytes &&
    typeof source.digest === "string" && base64.test(source.digest);
};
...
export const ownerVaultCatalogPages = (
...
): readonly { readonly entries: readonly OwnerVaultCatalogEntry[]; readonly digest: string; readonly bytes: number }[] | undefined => {
  ...
  const push = (): boolean => {
    ...
    if (digest === undefined || bytes === undefined || bytes > ownerVaultCatalogMaximumPageBytes) return false;
  ...
  for (const [ordinal, entry] of entries.entries()) {
    ...
    const candidateBytes = ownerVaultCatalogCanonicalBytes({ entries: candidate, digest: candidateDigest })?.byteLength;
    if (candidateDigest === undefined || candidateBytes === undefined) return undefined;
    if (candidate.length > ownerVaultCatalogMaximumPageEntries || candidateBytes > ownerVaultCatalogTargetPageBytes) {
...
export const ownerVaultCatalogEntryDigest = (value: unknown): string | undefined => ownerVaultCatalogDigest(value);
export const isOwnerVaultCatalogRevisionIdentifier = (value: string): boolean => revision.test(value);

export const ownerVaultCatalogWithinQuota = (entries: readonly Pick<OwnerVaultCatalogEntry, "bytes">[]): boolean =>
  entries.length <= ownerVaultCatalogMaximumObjects &&
  entries.every((entry) => safeNonNegativeInteger(entry.bytes) && entry.bytes <= ownerVaultCatalogMaximumObjectBytes) &&
