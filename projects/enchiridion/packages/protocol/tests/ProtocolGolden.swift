...
enum ProtocolGolden {
  static func main() throws {
    ...
    let serverChallenge = ServerHelloChallengeFrame(type: "serverHelloChallenge", protocolVersion: version, connectionNonce: try EnchiridionFrameID("AAAAAAAAAAAAAAAAAAAAAA"), issuedAt: try EnchiridionSignedTimestamp(1760000000000), expiresAt: try EnchiridionSignedTimestamp(1760000120000), ownerID: try EnchiridionOwnerID("owner-1"), vaultID: try EnchiridionVaultID("vault-1"), authEpoch: try EnchiridionAuthEpoch(3), credentialEpoch: try EnchiridionCredentialEpoch(4), generationEpoch: try EnchiridionGenerationEpoch(5))
    let hello = HelloFrame(type: "hello", protocolVersion: version, connectionNonce: try EnchiridionFrameID("AAAAAAAAAAAAAAAAAAAAAA"), resumeToken: try EnchiridionResumeToken("AAAAAAAAAAAAAAAAAAAAAA"), deviceID: deviceID, authEpoch: try EnchiridionAuthEpoch(3), deviceSignature: signature)
    guard try EnchiridionHelloSigningPayload.canonicalBytes(hello, challenge: serverChallenge, nowMilliseconds: 1760000000001).base64EncodedString() == "" else { throw GoldenFailure.mismatch("hello challenge vector") }
    guard (try? EnchiridionHelloSigningPayload.canonicalBytes(HelloFrame(type: "hello", protocolVersion: version, connectionNonce: try EnchiridionFrameID("AAAAAAAAAAAAAAAAAAAAAQ"), resumeToken: nil, deviceID: deviceID, authEpoch: try EnchiridionAuthEpoch(3), deviceSignature: signature), challenge: serverChallenge, nowMilliseconds: 1760000000001)) == nil else { throw GoldenFailure.mismatch("hello nonce mismatch accepted") }
    guard (try? EnchiridionHelloSigningPayload.canonicalBytes(hello, challenge: serverChallenge, nowMilliseconds: 1760000120000)) == nil else { throw GoldenFailure.mismatch("expired challenge accepted") }
    let challengedWire = EnchiridionServerWebSocketFrame.serverHelloChallenge(serverChallenge)
    guard try JSONDecoder().decode(EnchiridionServerWebSocketFrame.self, from: JSONEncoder().encode(challengedWire)) == challengedWire else { throw GoldenFailure.mismatch("server challenge WebSocket Codable round trip") }
    ...
    let frame = SyncChangeFrame(
      ...
      payloadSHA256: try EnchiridionSHA256Digest("a12871fee210fb8619291eaea194581cbd2531e4b23759d225f6806923f63222"),
      causalVersion: try EnchiridionNonNegativeInt(9),
      observedHighWater: try EnchiridionNonNegativeInt(17),
