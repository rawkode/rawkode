...
This command takes an existing plan (from `/prompt-workflows-plan`) and enhances each section with parallel research agents. Each major element gets its own dedicated research sub-agent to find:
...
<thinking>
Check for documented learnings from /prompt-workflows-compound. These are solved problems stored as markdown files. Spawn a sub-agent for each learning to check if it's relevant.
</thinking>

**LEARNINGS LOCATION - Check these exact folders:**

```
docs/solutions/           <-- PRIMARY: Project-level learnings (created by /prompt-workflows-compound)
├── performance-issues/
│   └── *.md
├── debugging-patterns/
│   └── *.md
...
2. **Learnings/Solutions sub-agents** - Relevant documented learnings from /prompt-workflows-compound
...
3. **Start `/prompt-workflows-work`** - Begin implementing this enhanced plan
...
- **`/prompt-workflows-work`** → Call the /prompt-workflows-work command with the plan file path
- **Deepen further** → Ask which sections need more research, then re-run those agents
- **Revert** → Restore from git or backup

## Example Enhancement

**Before (from /prompt-workflows-plan):**
