{
  programs.nushell = {
    ...
    shellAliases = {
      ai = ''
        GEMINI_API_KEY="op://Private/Google Gemini/password" op run --account my.1password.eu -- aichat'';
    ...
    environmentVariables = {
      ...
      PATH =
        "($env.PATH | split row (char esep) | prepend $'($env.HOME)/.nix-profile/bin' | append $'($env.NIX_LINK)/bin')";
