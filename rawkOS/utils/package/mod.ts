...
export const archInstall = async (
	packages: string[],
	options: { verbose?: boolean; onOutput?: (data: string) => void; onError?: (data: string) => void } = {},
) => {
	// Acquire the mutex before installing packages
	await packageMutex.acquire();
	
	try {
		// Emit the starting message through the callback
		if (options.onOutput) {
			options.onOutput(`Installing packages: ${packages.join(", ")}...\n`);
		} else {
		...
		if (options.onOutput) {
			options.onOutput(`Finished installing packages: ${packages.join(", ")}\n`);
		} else {
